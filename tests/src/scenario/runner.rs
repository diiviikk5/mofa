...
use crate::replay::{ReplayArtifact, ReplayRun};
...
impl ScenarioRunner {
    ...
    {
        Ok(self.run_with_replay(scenario).await?.report)
    }

    pub async fn run_with_replay<F, Fut>(&self, scenario: F) -> anyhow::Result<ReplayRun>
    where
        F: FnOnce(ScenarioContext) -> Fut,
        Fut: Future<Output = Result<(), String>>,
    {
        ...
        let artifact = ReplayArtifact::capture(&self.spec, &context, &report).await;

        Ok(ReplayRun { report, artifact })
    }
